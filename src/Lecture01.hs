...
howManyDigits n = let abs_n = abs n in if abs_n < 10 then "single" else if abs_n < 100 then "two-digit" else "three-digit or more"
...
digitsCount :: Int -> Int
digitsCount n =     
  let c = div (abs n) 10 in  
  if c == 0 then 1 else 1 + ( digitsCount c)
